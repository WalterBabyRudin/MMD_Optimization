...
def z_update(z0,g,H,rho):
    ...
    Constraint = [cp.norm(z - z0) <= rho, cp.sum(z)<=3]
...
N = 50
d = 10
x, y = sample_generate(N,d)
sigma = kernelwidthPair(x,y)
sigma = sigma/2
rho = 3
...
z = np.zeros([d,1])
idx = np.random.choice(d, 10, replace=False)
z[idx] = 1

print(z)

f_0, g_0, H_0 = MMD_grad_compute(x, y, z, sigma)

H_1 = get_near_psd(-H_0)

z_update(z,g_0,-H_1,rho)
